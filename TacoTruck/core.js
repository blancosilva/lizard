	...
	$.getJSON(urlJSON, function(json) {
			$.each(json.geonames,function(i,item) {
				$.ajax({
					type: "POST",
					url:  "myScript.php",
					data: "lat=" + position.coords.latitude + "&long=" + position.coords.longitude + "&area=" + item.name,
					success: function(msg) {
					msg = "lat=" + position.coords.latitude + "&long=" + position.coords.longitude + "&area=" + item.name,
					alert( "Data sent: " + msg);
					}
					});
